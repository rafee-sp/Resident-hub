...
const ResidentMessagesLog = ({ onClose, selectedResidentId }) => {

  const [residentId, setResidentId] = useState(selectedResidentId ?? null);
  ...
  useEffect(() => {
    ...
    setResidentId(selectedResidentId);
    setMessageLogs([]);
    getMessages(selectedResidentId, DEFAULT_PAGE, DEFAULT_PAGE_SIZE);
  }, [selectedResidentId, showFailedOnly]);


  const getMessages = async (residentId, page = 0, size = 10) => {
    setLoading(true);
    ...
    } = await api.get(`/residents/${residentId}/messages`, { params });
  ...
  const handlePagination = (pageNumber, pageSize) => {
    getMessages(residentId, pageNumber, pageSize);
  };
          ...
          <table className="min-w-full bg-white shadow  overflow-hidden">
            <thead className="bg-blue-900 text-center text-white text-lg z-10">
              <tr>
                ...
                <th className="px-4 py-2 w-1/5">Message Type</th>
                ...
                {messageLogs && messageLogs.length > 0 ? (
                  messageLogs.map((message) => (
                    <tr key={message.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3"><BroadcastMode mode={message.broadcastMode} /></td>
...
export default ResidentMessagesLog;
